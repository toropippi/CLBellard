...
{
	//uint2 d=ultou2(uld);
	uint2 v4,hi,lo,v3,ehat,v2,v1, d40, d63;
	uint v0,d0,d9;
	/*
	const uint table[] = {
	// Generated with:
	...
	*/
	...
	d0 = d.x & 1;//[0,2)
	d9 = d.y>>23;// [256,512)
	d40 = uladd(u2shr(d,24),(uint2){1,0});// (2^39,2^40]
	d63 = uladd(u2shr(d,1),(uint2){d0,0});// (2^62,2^63]
	v0 = CreateTable(d9 - 256);
	//v0 = table[d9 - 256];
	//v0は高々2047
	uint2 tmp=u2mymullo128((uint2){v0*v0,0},d40);
	v1.y=0;
	v1.x = (v0<<11)-1-(tmp.y>>8);
	
	v2 = uladd(u2shl(v1,13) , u2shr(u2mymullo128(v1, ulsub((uint2){0,1<<28} , u2mymullo128(v1, d40))) , 47));
	ehat.x=0;ehat.y=0;
	...
	v4 = ulsub(v3 , uladd(uladd(hi,d),(uint2){out3.z,0}));
	return v4;
}
...
{
	//uint2 u1=ultou2(u1ul);
	//uint2 u0=ultou2(u0ul);
	//uint2 d=ultou2(dul);
	//uint2 v=ultou2(vul);
	uint2 u2r;//=ultou2(*r);
	uint2 q0, q1;
	...
	q0=out3.xy;
	//q0 = q0 + u0;
	q1 = uladd(uladd(q1,u1),(uint2){out3.z+1,0});
	//q1++;
	u2r=ulsub(u0,u2mymullo128(q1, d));
	//*r = u0 - mymullo128(q1, d);
	if (gt(u2r,q0)){
		q1=ulsub(q1,(uint2){1,0});
		u2r=uladd(u2r,d);
	}
	//q1 = (*r > q0) ? q1 - 1 : q1;
	//*r = (*r > q0) ? *r + d : *r;
...
void divnby1(int n, const uint2 *u, uint2 d, uint2 *q)
{
	...
	for (i = n - 1; i >= 1; i--) {
		...
		out4 = div2by1(k, ui, d, v);
		q[i] = out4.xy;
		k = out4.zw;
	}
	
	out4 = div2by1(k, u2shl(u[0],l), d, v);
	q[0] = out4.xy;
	//k=out4.zw;
}
...
__kernel void Sglobal64mtg_192_Refresh(__global ulong *bigSum,const ulong offset,const ulong d,const ulong k_max,const long numesign,const ulong den0,const ulong den1) {
	...
	ulong ulsum1=0;//中間
	ulong ulsum2=0;//最下位桁
	uint2 ulsum0_=0;//最上位桁
	uint2 ulsum1_=0;//中間
	uint2 ulsum2_=0;//最下位桁
	uint3 out3;
	
	for(;k<k_max;k+=gsize)
	{
		nmr=powmodn((uint2){1024,0},ultou2(d-k),ultou2(dnm));
		//k^-1のところ。答えを適宜反転
		if ((numesign+k%2)%2==1){
			nmr=ulsub(ultou2(dnm),nmr);
		}
		
		uint2 u[4]={0,0,0,nmr};
		uint2 q[4];
		divnby1(4,u,ultou2(dnm),q);
		/*
		ulong q[4];
		for(int i=0;i<4;i++){
			q[i]=u2toul(qq[i]);
		...
		*/
		...
		ulsum0_=uladd(ulsum0_,q[2]);
		dnm+=dnmadd;
	}
	//長いループ終了
	ulsum0=u2toul(ulsum0_);
	ulsum1=u2toul(ulsum1_);
	ulsum2=u2toul(ulsum2_);
	
	__local uint2 p[128*3];
